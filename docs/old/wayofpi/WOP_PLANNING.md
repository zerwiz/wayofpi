...
| **[WOP_OPEN_TODOS.md](WOP_OPEN_TODOS.md)** | **Living backlog** — not-done items across product, **`apps/wayofwork-ui`**, scripts, docs |
...
| **[WOP_TECHNICAL_UI.md](WOP_TECHNICAL_UI.md)** | **`apps/wayofwork-ui`** technical shell — **`WorkspacePane`** (Zed-style **file ∪ tools** tab stack), optional **`TechnicalWorkspaceGrid`** (≤ **3×4**, flex + splitters, **`rowWeights`/`colWeights`**, edge-grow + cross-cell tab DnD), persistence (**`workspaceGrid.v1`**, **`panelDock`**, **`dockLayout`**); modular **N** strips / full pane graph still planned |
| **[WOP_MOBILE_UI_PLAN.md](WOP_MOBILE_UI_PLAN.md)** | **Mobile layouts** for **Claw**, **Simple**, **Technical** (distinct per shell); **ship order: Claw → Simple → Technical**; **`?shell=mobile`**, same **`surfaceId`** / **`/ws`** spine |
...
| **[WOP_EXTENSION_APPEARANCES_VIEWS_PLAN.md](WOP_EXTENSION_APPEARANCES_VIEWS_PLAN.md)** | **`pi-e` menu / extension stacks** → **`wayofwork-ui`** **views** via **dock presets** (inventory table, phases, open questions) |
...
**Implementation preview (web shell):** [apps/wayofwork-ui/README.md](../apps/wayofwork-ui/README.md), **[WOP_TECHNICAL_UI.md](WOP_TECHNICAL_UI.md)**.
