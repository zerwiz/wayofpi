...
import { tunnelGateAllowsNodeRequest, tunnelGateWriteUnauthorizedNode } from "./server/tunnel-gate";
...
function attachTunnelHostForward(proxy: {
	...
	const set = (proxyReq: { setHeader: (n: string, v: string) => void }, req: IncomingMessage) => {
		const h = req.headers.host;
		if (h) proxyReq.setHeader("X-Forwarded-Host", h);
	};
	proxy.on("proxyReq", (proxyReq, req) => set(proxyReq as { setHeader: (n: string, v: string) => void }, req as IncomingMessage));
	proxy.on("proxyReqWs", (proxyReq, req) => set(proxyReq as { setHeader: (n: string, v: string) => void }, req as IncomingMessage));
}
...
const vitePortEnv = String(process.env.WOP_VITE_PORT ?? process.env.VITE_DEV_SERVER_PORT ?? "").trim();
