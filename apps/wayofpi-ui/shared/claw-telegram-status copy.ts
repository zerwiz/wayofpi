...
const TOKEN_SOURCES: ClawTelegramTokenSource[] = ["none", "workspace", "global", "both"];

/** Validate JSON from **`GET /api/config`** → **`clawTelegramStatus`** or the dedicated status route. */
export function parseClawTelegramStatusV1(raw: unknown): ClawTelegramStatusV1 | null {
	if (!raw || typeof raw !== "object") return null;
	const o = raw as Record<string, unknown>;
	if (o.version !== CLAW_TELEGRAM_STATUS_VERSION) return null;
	const ts = o.tokenSource;
	if (typeof ts !== "string" || !TOKEN_SOURCES.includes(ts as ClawTelegramTokenSource)) return null;
	for (const k of ["globalTokenConfigured", "workspaceTokenConfigured", "piTelegramInSettings"] as const) {
