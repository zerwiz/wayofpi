...
export function isOutputPanelLog(row: LogStreamRow): boolean {
	if (row.source === "read" && row.msg.startsWith(READ_WORKSPACE_PREFIX)) return true;
	if (row.source === "write" && row.msg.startsWith(WRITE_WORKSPACE_PREFIX)) return true;
...
export const HOST_WORKSPACE_API_TOOL_SOURCES = ["read", "write", "bash", "mv", "mkdir"] as const;
