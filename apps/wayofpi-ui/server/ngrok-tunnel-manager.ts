...
/** Run package install in **apps/wayofpi-ui** to fetch optional **ngrok** (no `sudo`; separate from spawn gate). */
...
export type NgrokTunnelGetJson = {
	...
	/** Server may run **bun install** / **npm install** in **apps/wayofpi-ui** to fetch optional **ngrok** */
...
export async function startNgrokTunnelDev(): Promise<NgrokTunnelActionJson> {
	...
	if (!listening) {
		return {
			ok: false,
			message: `Nothing is listening on 127.0.0.1:${port}. Start the dev server first (e.g. Vite on port ${port} — usually "npm run dev" / ./start-wayofpi-ui.sh with Bun on 3333). ngrok needs a live backend or the public URL will fail (ngrok ERR_NGROK_3200 / offline after you stop the tunnel). For Bun-only static + API, set WOP_VITE_PORT or VITE_DEV_SERVER_PORT to your HTTP port (often 3333) so the tunnel target matches.`,
		};
	}
...
async function runNgrokBundledPackageOp(op: BundledNgrokPackageOp): Promise<NgrokTunnelActionJson> {
	...
	if (!existsSync(pkgJson)) {
		return { ok: false, message: "Could not find apps/wayofpi-ui (package.json missing next to the server)." };
	}
	...
	return {
		ok: true,
		message:
			"Updated the optional ngrok package in apps/wayofpi-ui. Press Refresh; restart the Bun server if the version line still looks stale. A system ngrok on PATH is unchanged.",
	};
}
...
/** Upgrade the optional **ngrok** npm package under **apps/wayofpi-ui** (does not replace a PATH binary). */
