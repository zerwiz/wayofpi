...
/**
 * Directory to run ESLint / tsc in: workspace root, or **apps/wayofpi-ui** when this monorepo is opened at repo root.
 */
function pickAnalysisDirectory(rootAbs: string): string | null {
	...
	const nested = join(rootAbs, "apps", "wayofpi-ui");
 ...
 * Uses the workspace root, or **apps/wayofpi-ui** when the repo root has no config but that package does (monorepo).
 */
export async function runWorkspaceProblemsAnalysis(rootAbs: string): Promise<WorkspaceProblemsRunResult> {
	...
	if (!analysisCwd) {
		return {
			...
			problems: [],
			exitCode: null,
			log: "No eslint config or tsconfig.json at the workspace root (or under apps/wayofpi-ui for this layout). Add one to enable static checks.",
		};
	}
