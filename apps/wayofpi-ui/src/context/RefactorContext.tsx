...
export const RefactorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const effDirty = isWsMulti ? multiCellAnyDirty : editor.dirty;
  const effFileLoading = isWsMulti ? !!techWsSnapshot?.loading : editor.loading;
  const effFileError = isWsMulti ? techWsSnapshot?.error ?? null : editor.error;
  ...
  const value = useMemo(() => ({
    ...
    effSelectedPath, effDirty, effFileLoading, effFileError,
