...
let page: ReactNode;
if (!hasToken && (path === "/" || path === "/welcome")) {
	page = <WelcomePage />;
} else if (path === "/login" || (!hasToken && !["/portal", "/client", "/admin", "/super-admin", "/profile"].some((p) => path === p || path.startsWith(p + "/")))) {
	page = <LoginPage />;
} else {
	page = <ErrorBoundary><App /></ErrorBoundary>;
}
