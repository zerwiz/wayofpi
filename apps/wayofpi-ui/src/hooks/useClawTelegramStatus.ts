...
async function loadTelegramStatus(): Promise<LoadResult> {
	...
	try {
		...
		return {
			...
			issue: "network",
			message:
				`Could not load server settings (${message}). Start the Way of Pi Bun API (same checkout as this UI), e.g. \`cd apps/wayofpi-ui && bun run dev\` or \`bun run server/index.ts\`, then refresh.`,
		};
	}
	...
	return {
		...
		issue: "stale_api",
		message:
			"The process answering on your API port is an older Way of Pi Bun build (no Telegram snapshot in /api/config). Stop that process, then start the server from this checkout — e.g. `cd apps/wayofpi-ui && bun run dev` or `bun run server/index.ts` — and click Refresh status.",
	};
}
