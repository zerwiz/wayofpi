...
import {
  ...
  type WorkspaceGridPickerConfig,
} from "./WorkspaceGridLayoutPicker";
...
import {
  firstMarkdownHeadingLine,
  workspacePathBreadcrumbSegments,
} from "../utils/workspaceEditorChrome";
...
function PanelTabIcon({ entry, active }: { entry: PanelTab; active: boolean }) {
  ...
  if (entry.type === "file") {
    return (
      <FileCode2 size={14} className="shrink-0 text-[#519aba]" aria-hidden />
    );
  }
  switch (entry.id) {
    case "problems":
    ...
    case "agent_chat":
      return (
        <MessageSquare
          size={14}
          className={active ? "text-[#cccccc]" : "text-[#858585]"}
          aria-hidden
        />
      );
  }
...
export const WorkspacePane = forwardRef<WorkspaceEditorRef, WorkspacePaneProps>(
  function WorkspacePane(
    ...
    const [visualMediaMode, setVisualMediaMode] = useState<
      "preview" | "source"
    >("preview");
    ...
    const onBarDrop = (e: React.DragEvent) => {
      ...
      const path =
        rawPath.length > 0
          ...
          : parseFilePathDragJson(e.dataTransfer.getData("text/plain"));
      const before = hintToBeforeTab(dropHintRef.current);
      if (moving) {
    ...
    const safeIndex =
      entries.length === 0
        ? 0
        : Math.min(Math.max(0, activeIndex), entries.length - 1);
    ...
    const showBread = showBreadcrumbs && showDocChrome;

    const mediaDualChrome = Boolean(
      showDocChrome &&
      filePreview &&
    ...
    const mediaSegOn =
      "rounded-md border border-[#ea580c]/60 bg-[#3c3c3c] px-2.5 py-1 font-mono text-[11px] text-[#fed7aa]";
    const mediaSegOff =
    ...
    const pathSegments = editorPath
      ? workspacePathBreadcrumbSegments(editorPath)
      : [];
    const isMarkdownUtf8 = Boolean(
      showDocChrome &&
    ...
    const mdHeading =
      isMarkdownUtf8 && showBread ? firstMarkdownHeadingLine(content) : null;
    const fileTabIndices = entries
      .map((e, i) => (e.type === "file" ? i : -1))
      .filter((i): i is number => i >= 0);
    ...
    const showGitFileReviewChrome =
      Boolean(gitFileReviewActions) && gitReviewHasAnyMarked;
    /** Tab-cycling review (2+ file tabs) or Git queue review (marked paths in workspace tree). */
    const showFileReviewChrome =
      canNextFileForReview || showGitFileReviewChrome;
          ...
          >
            <div className="inline-flex h-9 min-w-max max-w-none flex-nowrap items-stretch">
              {entries.map((entry, idx) => {
                ...
                const fileDirty =
                  entry.type === "file" && dirty && editorPath === entry.path;
            ...
            <div className="hidden h-full items-center gap-0.5 md:flex">
              {workspaceDockActions ? (
                <>
                  ...
                  <button
                    ...
                    onClick={() =>
                      workspaceDockActions.onShowAgentChat(
              ...
              {onCloseWorkspacePane ? (
                <button
                  ...
                  aria-label={
                    workspaceMaximizeActive
                      ? "Exit maximized workspace view"
                      : "Close workspace cell"
                  }
                >
                  <X size={14} strokeWidth={2} />
                </button>
            ...
            >
              ...
              {mobilePaneToolsOpen ? (
                ...
                >
                  ...
                  <li
                    role="none"
                    className="mb-1 border-b border-[#3c3c3c] pb-1"
                  >
                    <button
                      type="button"
                      role="menuitem"
        ...
        {showDocChrome ? (
          ...
          >
            {showBread ? (
              <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-1 gap-y-1 font-mono text-[11px] text-[#d4d4d4]">
                {breadcrumbWorkspaceLabel ? (
                  <>
                    <span
                      ...
                      title={breadcrumbWorkspaceLabel}
                    >
                      {breadcrumbWorkspaceLabel}
                    </span>
                    ...
                    <span
                      className={
                        i === pathSegments.length - 1
                          ? "truncate font-medium text-[#fed7aa]"
                          : "truncate"
                      }
                    >
                      {seg}
                ...
                {mdHeading ? (
                  <>
                    ...
                    <span
                      className="min-w-0 truncate text-[#a3a3a3]"
                      title={mdHeading}
                    >
                      {mdHeading}
                    </span>
              ...
              <div
                className="min-w-0 flex-1 truncate font-mono text-[11px] text-[#fed7aa]"
                title={editorPath ?? ""}
              >
              ...
              {isMarkdownUtf8 ? (
                ...
                >
                  <button
                    ...
                    className={
                      markdownViewMode === "preview" ? mediaSegOn : mediaSegOff
                    }
                  >
                    Preview
                  </button>
                  <button
                    ...
                    className={
                      markdownViewMode === "source" ? mediaSegOn : mediaSegOff
                    }
                  >
                    Source
                  </button>
                ...
                >
                  <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    <button
                      type="button"
                      disabled={
                        gitFileReviewActions ? !gitReviewCanAdvanceNext : false
                      }
                      onClick={() =>
                        void (gitFileReviewActions
                  ...
                  <div className="flex items-center gap-1 font-mono text-[11px] text-[#858585]">
                    ...
                    <span
                      ...
                      title="Open file tabs in this workspace pane"
                    >
        ...
        {mediaDualChrome ? (
          ...
          >
            ...
            <button
              ...
              className={
                visualMediaMode === "preview" ? mediaSegOn : mediaSegOff
              }
            >
              Preview
            </button>
            <button
              ...
              className={
                visualMediaMode === "source" ? mediaSegOn : mediaSegOff
              }
            >
              Source
            </button>
          ...
          {!activeEntry ? (
            ...
            >
              <p className="max-w-md font-mono text-[12px] leading-relaxed text-[#858585]">
                <strong className="text-[#cccccc]">Workspace</strong> — add a
                tab with + (files, terminal, problems, …). Same pane type can be
                split later.
              </p>
              {showExplorerHint ? (
                <p className="max-w-sm font-mono text-[11px] text-[#6b6b6b]">
                  Use + or open a file from the explorer.
                </p>
              ) : null}
              {onOpenWorkspace ? (
                <button
            ...
            <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
              {workspaceEmbeddedChat ? (
                ...
                <p className="p-3 font-mono text-[12px] text-[#858585]">
                  Agent chat is not wired for this pane.
                </p>
              )}
            </div>
            ...
            <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
              {agentTeamPane ? (
                ...
                <p className="p-3 font-mono text-[12px] text-[#858585]">
                  Team pulse data not wired for this pane.
                </p>
              )}
            </div>
            ...
            <div className="flex flex-1 items-center justify-center p-6 font-mono text-[13px] text-[#f14c4c]">
              {error}
            </div>
          ) : showMediaAsSource ? (
            <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
              {persistEncoding === "base64" ? (
                <div className="shrink-0 border-b border-[#2d2d2d] bg-[#252526] px-3 py-1 font-mono text-[10px] text-[#858585]">
                  Byte editor (Latin-1) — colors off; Save writes the exact file
                  bytes.
                </div>
              ) : null}
              <WorkspaceTextBuffer
            ...
            <div className="flex min-h-0 flex-1 overflow-auto bg-[#1e1e1e] p-4 font-mono text-[13px] leading-relaxed text-[#d4d4d4]">
              <div className="max-w-xl text-[#858585]">
                ...
                <p>
                  Type{" "}
                  <span className="text-[#fed7aa]">{filePreview.mimeType}</span>
                  . This file is not shown as text. Open it externally or use
