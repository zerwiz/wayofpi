...
type NgrokTunnelGetJson = {
	...
	inspectorUrl?: string;
	/** Server can run bun/npm install in apps/wayofpi-ui to fetch optional ngrok */
...
function ngrokSourceDescription(source: NgrokResolvedSource): string | null {
	if (source === "bundled")
		return "Optional bundled agent (npm package ngrok in apps/wayofpi-ui - install deps, or skip ngrok entirely).";
		...
		return (
			<div className={`mb-4 rounded-lg border p-4 ${card}`}>
				...
				<ol className={`mt-3 list-decimal space-y-1.5 pl-5 text-xs leading-relaxed ${desc}`}>
					<li>
						Start the API: <Mono dark={dark}>cd apps/wayofpi-ui && bun run server/index.ts</Mono> — or from the repo root run{" "}
						<Mono dark={dark}>./start-wayofpi-ui.sh</Mono> (Vite + Bun).
					</li>
					<li>
	...
	if (loadState === "dev" && allowControl && !ngrokOnPath) {
		return (
			<div className={`mb-4 rounded-lg border p-4 ${card}`}>
				<h3 className={`text-sm font-bold ${titleC}`}>3. Tunnel</h3>
				<p className={`mt-1 text-xs leading-relaxed ${desc}`}>
					Run <strong className={dark ? "text-[#e0e0e0]" : "text-[#111]"}>bun install</strong> in <Mono dark={dark}>apps/wayofpi-ui</Mono>, or install
...
export function NgrokSettingsModal({
	...
	/** Server-side optional **ngrok** package in apps/wayofpi-ui (`install-bundled` / `update-bundled`). */
							...
							{tunnelStatus.ngrokOnPath ? (
									...
									{tunnelStatus.installBundledAllowed ? (
										<div className="mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
											<button
												...
												title="Runs bun update ngrok or npm update ngrok in apps/wayofpi-ui on the server. Does not upgrade a system ngrok on PATH."
											>
								...
								<>
									<p className={`mb-3 text-[12px] leading-relaxed ${sub}`}>
										...
										<Mono dark={appearanceDark}>apps/wayofpi-ui</Mono> so the optional <Mono dark={appearanceDark}>ngrok</Mono> package can place the
  ...
  && sudo apt update && sudo apt install -y ngrok`}
					...
					{tunnelLoadState === "dev" && tunnelStatus && !backendListening ? (
						...
						>
							...
							Start the dev server first (Vite on that port with Bun on 3333, or your usual ./start-wayofpi-ui.sh). Way of Pi will refuse to start the
					...
					<details className={`mb-4 rounded-lg border p-4 ${card}`}>
						...
						<div className={`mt-3 space-y-3 text-[12px] leading-relaxed ${sub}`}>
							<p>
								...
								<Mono dark={appearanceDark}>apps/wayofpi-ui</Mono>), then runs <Mono dark={appearanceDark}>ngrok config add-authtoken</Mono> and{" "}
						...
						>
							...
							<div className="flex flex-wrap gap-1">
								...
								<button
									type="button"
									className={injectChipBtn}
									title="Inserts: cd apps/wayofpi-ui && bun install (optional ngrok package; same as Settings install button; use npm install --no-fund --no-audit if you do not use bun)"
									onClick={() => injectIntoTerminal("cd apps/wayofpi-ui && bun install")}
								>
									+ install ngrok
								</button>
								<button
									type="button"
									className={injectChipBtn}
									title="Runs: cd apps/wayofpi-ui && bun install (downloads optional ngrok into node_modules when listed in package.json)"
									onClick={() => injectIntoTerminal("cd apps/wayofpi-ui && bun install\r")}
								>
									Run install
								</button>
								<button
									type="button"
									className={injectChipBtn}
									title="Inserts: cd apps/wayofpi-ui && bun update ngrok (optional package; use npm update ngrok if you do not use bun)"
									onClick={() => injectIntoTerminal("cd apps/wayofpi-ui && bun update ngrok")}
								>
									+ update ngrok
								</button>
								<button
									type="button"
									className={injectChipBtn}
									title="Runs: cd apps/wayofpi-ui && bun update ngrok"
									onClick={() => injectIntoTerminal("cd apps/wayofpi-ui && bun update ngrok\r")}
								>
									Run update
								</button>
								<button
									type="button"
									className={injectChipBtn}
									title="Runs: cd apps/wayofpi-ui && bun run server/index.ts (from repo root layout)"
									onClick={() => injectIntoTerminal("cd apps/wayofpi-ui && bun run server/index.ts\r")}
								>
									Start Bun API
								</button>
							</div>
