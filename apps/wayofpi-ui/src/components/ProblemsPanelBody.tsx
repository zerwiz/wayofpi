		...
		try {
			...
			if (shell?.startWayOfPiBunServer) {
				...
				if (r.staleServer) {
					setStartHint(
						r.message ??
							"An older Bun server is still on port 3333. Stop that process, then start again from apps/wayofpi-ui.",
					);
					return;
				}
			...
			setStartHint(
				copied
					...
					: "Could not copy to clipboard. Run from repo root: cd apps/wayofpi-ui && bun run server/index.ts",
			);
							...
							<button
								...
								title={
									typeof window !== "undefined" && window.wopShell?.startWayOfPiBunServer
										? "Spawn Bun API (apps/wayofpi-ui) for Vite to proxy /api — Electron dev."
										: import.meta.env.DEV
					...
					<div className="space-y-2 p-3 font-mono text-[12px] leading-relaxed text-[#858585]">
						{!ok ? (
							...
							<p>
								...
								<strong className="text-[#cccccc]">apps/wayofpi-ui</strong> in this monorepo) needs an ESLint config or{" "}
