...
export function WorkspaceGridLayoutPicker({
  ...
  useEffect(() => {
    ...
    const close = (e: MouseEvent) => {
      if (rootRef.current && !rootRef.current.contains(e.target as Node)) {
        console.log("[WorkspaceGridLayoutPicker] mousedown outside, closing.");
        setOpen(false);
      }
    };
  ...
  console.log("[WorkspaceGridLayoutPicker] render, open:", open);
  ...
  return (
    ...
    >
      <button
        ...
        onClick={() => {
          console.log(
            "[WorkspaceGridLayoutPicker] button clicked, toggling open state.",
          );
          setOpen((v) => !v);
        }}
        className={
      ...
      >
        <LayoutGrid
          size={isMenu ? 14 : 16}
          strokeWidth={2}
          className={isMenu ? "shrink-0 text-[#858585]" : undefined}
        />
        {isMenu ? (
