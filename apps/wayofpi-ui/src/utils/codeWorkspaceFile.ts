 ...
 * Relative `folders[].path` values resolve from the directory containing the workspace file
 * (mirrors server `loadFoldersFromWorkspaceJson` in `server/workspace-state.ts`).
 */
...
export function buildCodeWorkspacePayload(
	...
	for (const f of folders) {
		...
		const path =
			base != null ? unixRelativePath(base, abs) : abs;
