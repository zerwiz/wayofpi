...
- **ngrok (optional):** **`ngrok`** is an **optionalDependency** in **`apps/wayofpi-ui/package.json`**. A normal **`bun install`** / **`npm install`** there can download the official agent into **`node_modules/ngrok/bin`**; Way of Pi’s Settings → ngrok uses it when **`WOP_NGROK_BINARY`** and PATH do not already provide one. Skip it with **`npm install --omit=optional`** (or omit the package) if you never use public tunnels. **Tunnel login (optional):** Settings → ngrok → **Tunnel login** stores **`tunnel-gate.v1.json`** under **`WOP_HOME`** and enforces **HTTP Basic Auth** on **ngrok-style** hostnames for the **Vite dev server** and the **Bun** API (including **`/ws`**). **Canonical doc:** **[docs/WOP_NGROK.md](../../docs/WOP_NGROK.md)**.
...
**Mobile layout (touch-first, same session keys):** append **`?shell=mobile`** to the dev URL, or open the app at path **`/m`** (SPA). The flag is mirrored to **`localStorage`** as **`wayofpi.shell.mobile`** (`1` / `0`). Implementation lives under **`src/components/mobile/`** (**`chrome/`**, **`claw/`**, **`simple/`**, **`technical/`**, **`useShellMobile.ts`**, **`index.ts`** — see **`src/components/mobile/README.md`**). **Claw** on mobile uses a bottom tab bar (instead of the left rail), a **Sessions** bottom sheet and full-screen **Workspace** overlay in **Chat**, stacked **Files** tree + editor on the **Files** tab, **`100dvh`** help with horizontal section chips, and the same **`useWayOfPiSession("claw")`** / **`/ws`** paths as desktop. **Simple** uses **`SimpleApp`** with **`layoutVariant="mobile"`** (bottom **`SimpleMobileTabBar`**, project-files sheet, editor overlay) — same **`useWayOfPiSession("simple")`** as desktop. **Technical** still uses a stub shell until Track 3 (**[docs/WOP_MOBILE_UI_PLAN.md](../../docs/WOP_MOBILE_UI_PLAN.md)**). Use **Desktop** in the mobile top bar (or **`?shell=desktop`**) to return to the full chrome.

**Narrow Simple (desktop shell, viewport ≤767px):** without **`?shell=mobile`**, **`SimpleApp`** still uses **`layoutVariant="desktop"`** but matches mobile-style chat chrome (**`compactChrome`**), a **Project files** strip + **right** tree sheet, a **left** editor slide-over over the chat column, and the same **menu / palette** file-focus bump as the mobile shell so global opens (e.g. **Save as**) keep the editor in sync after navigation or resize.
...
- **`npm run electron:only`** — Electron only; use when **`npm run dev`** is already running (avoids **EADDRINUSE** on **3333**). In **`ELECTRON_DEV=1`**, Electron **auto-starts the Bun API** on launch if **`/api/health`** is not already “fresh”; set **`WOP_ELECTRON_SKIP_BUN_AUTOSTART=1`** to disable.
...
| GET | `/api/dev/share-url-hints` | JSON **`lanUrl`** (best-effort `http://<IPv4>:<WOP_VITE_PORT|5173>/`), **`ngrokPublicUrl`** (from ngrok local inspector **`http://127.0.0.1:4040/api/tunnels`** when the ngrok agent is running). **Settings → ngrok (public URL)…** uses this for copy-paste (LAN + public). Served in all **`NODE_ENV`**; **404** means the request did not reach this Bun server. |
| GET | `/api/dev/ngrok-tunnel` | JSON **`allowControl`**, **`authtokenSaveAllowed`**, **`wopManagedRunning`**, **`pid`**, **`tunnelPort`**, **`publicUrl`**, **`backendListening`** (TCP on **`127.0.0.1:tunnelPort`**), **`inspectorUrl`** (ngrok web UI; **`WOP_NGROK_WEB_ADDR`**), **`ngrokOnPath`**, **`ngrokVersionLine`**, **`ngrokWhichPath`**, **`ngrokResolvedSource`** (`WOP_NGROK_BINARY` \| `PATH` \| `bundled`). Authtoken save, **`install-bundled`**, and **`update-bundled`** are independent of **`WOP_ALLOW_NGROK_SPAWN`**; **`start`**/**`stop`** respect that flag. **`start`** requires **`backendListening`** before spawn. |
| POST | `/api/dev/ngrok-tunnel` | JSON **`{ "action": "start" \| "stop" \| "install-bundled" \| "update-bundled" \| "set-authtoken", "authtoken"?: string }`** — **`install-bundled`** runs **`bun install`** / **`npm install`** in **`apps/wayofpi-ui`**; **`update-bundled`** runs **`bun update ngrok`** / **`npm update ngrok`** (optional package only; does not replace a **`PATH`** binary). |
| GET | `/api/dev/tunnel-gate` | JSON **`loginEnabled`**, **`configured`**, **`username`**, **`wopHome`**, **`configPath`**, **`publicHostHint`** — tunnel HTTP Basic Auth (same file Vite + Bun read). **Settings → ngrok → Tunnel login**. |
| POST | `/api/dev/tunnel-gate` | JSON **`{ "action": "save" \| "clear", "enabled"?: boolean, "username"?: string, "password"?: string }`** — write or remove **`tunnel-gate.v1.json`** under **`WOP_HOME`**. |
