...
| Method | Path | Purpose |
|--------|------|---------|
| GET | `/api/health` | Liveness; **`capabilities.workspaceProblems`** and **`capabilities.configRuntimePost`** (both must be **true** on current builds) — Vite/Electron dev “fresh” detection uses them so a stale Bun on the port is not treated as healthy. |
...
| GET | `/api/config` | LLM provider, **`chatEngine`**, **`piBinaryResolved`**, **`piChatEngineRequested`** / **`piChatEngineWired`**, **`piDrivesChat`**, interim **`orchestratorTools`** / **`orchestratorBash`**, **`manifestUrl`**, models, **`terminalEnabled`**, plus **`capabilities`** (same keys as **`/api/health`**: **`workspaceProblems`**, **`configRuntimePost`**) for UI stale-server detection — see **[WOP_PI_BACKEND_WIRING_PLAN.md](../../docs/WOP_PI_BACKEND_WIRING_PLAN.md)** §0, **[docs/WOP_ORCHESTRATION_EXTENSIONS_PANEL.md](../../docs/WOP_ORCHESTRATION_EXTENSIONS_PANEL.md)**. |
