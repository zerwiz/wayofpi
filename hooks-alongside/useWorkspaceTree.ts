...
export interface WorkspaceTreeNode {
  ...
  gitStatus?: "untracked" | "modified" | "added" | "deleted" | "renamed";
}
...
export function useWorkspaceTree(): UseWorkspaceTreeReturn {
  ...
  const setFolders = useCallback((newFolders: string[]) => {
    setFoldersState((prev) => newFolders);
    try {
    ...
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === STORAGE_KEYS.ROOT) {
        ...
        try {
          const stored = event.newValue;
          if (stored) {
            setFoldersState(JSON.parse(stored) as string[]);
          }
        } catch {
          // Ignore invalid JSON
        }
