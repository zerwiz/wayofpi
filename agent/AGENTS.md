...
3. Populate overview and context from the **target repo** (scan README, manifests, layout) or **`dispatch_agent`** → **`project-scanner`** with the workspace **absolute path** (team **`new-project`**). For a **durable file tree + per-file purpose map** at the codebase root, **`dispatch_agent`** → **`indexer`** (teams **`index`**, **`new-project`**, **`full`**, **`info`**) — output **`INDEX.md`**; other agents should **`read`** it first. To **embed** a playground extension/skill into the **target repo** (not only link it), use **`playground-portal`** (teams **`playground-portal`**, **`new-project`**, **`full`**, **`info`**) after **`pi-e` option 2** or **`init-project-local-pi-env.sh`**.
