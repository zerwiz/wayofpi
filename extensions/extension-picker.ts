 ...
 *
 * Commands:
 *   /extensions — pick an extension (prefix /ex matches in the command menu); saves `pi -e <path>` to storage
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		ctx.ui.notify(
			`Extension picker: ${entries.length} entries (${nPkg} package(s) + project). Type /extensions (or /ex… to filter)`,
			"info",
		);
	});
	...
	pi.registerCommand("extensions", {
		description:
			"Extension picker: project + package entrypoints (type /ex in menu to narrow). Saves pi -e for next launch",
